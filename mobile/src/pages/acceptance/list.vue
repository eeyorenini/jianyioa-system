<template>
  <view class="page">
    ...
    <view class="nav-bar">
      ...
      <text class="nav-title">验收管理</text>
    ...
    <view class="filter-row">
      ...
      <view class="filter-tag" :class="{ active: curStatus === '待验收' }" @click="curStatus = '待验收'">待验收</view>
      <view class="filter-tag success" :class="{ active: curStatus === '已通过' }" @click="curStatus = '已通过'">已通过</view>
      <view class="filter-tag danger" :class="{ active: curStatus === '整改中' }" @click="curStatus = '整改中'">整改中</view>
    </view>
    ...
    <scroll-view class="list-area" scroll-y @scrolltolower="loadMore">
      ...
      <view v-else-if="list.length === 0" class="empty-state">
        ...
        <text class="empty-text">暂无验收记录</text>
      </view>
      <view v-else>
        <view class="record-card" v-for="item in filteredList" :key="item.id" @click="goDetail(item)">
          <view class="card-top">
            <view class="card-title">{{ item.title || item.item_name || '验收项' }}</view>
            <view class="status-pill" :class="getStatusClass(item.result)">{{ item.resultText }}</view>
          </view>
          <view class="card-meta">
            <text>📋 {{ item.check_type || '常规验收' }}</text>
            <text>👤 {{ item.inspector_name || item.inspector || '未知' }}</text>
            <text>📅 {{ formatDate(item.check_date || item.created_at) }}</text>
          </view>
...
const filteredList = computed(() => {
  ...
  return list.value.filter(i => i.resultText === curStatus.value);
});

const getStatusClass = (result) => {
  if (result === 'passed' || result === '已通过') return 's-pass';
  if (result === 'rework' || result === '整改中') return 's-rework';
  return 's-pending';
};
...
const goDetail = (item) => {
  uni.navigateTo({ url: `/pages/acceptance/detail?id=${item.id}&projectId=${projectId.value}` });
};

const goAdd = () => {
  uni.navigateTo({ url: `/pages/acceptance/add?projectId=${projectId.value}` });
};
...
.s-rework { background: #FEE2E2; color: #991B1B; }
