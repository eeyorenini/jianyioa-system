<template>
  <div class="contract-templates-editor">
    ...
    <!-- 编辑对话框 -->
    <el-dialog v-model="showDialog" :title="form.id?'编辑模板':'新增模板'" width="90%" top="2vh" destroy-on-close>
      <div v-loading="uploadLoading" element-loading-text="正在导入PDF..." class="editor-container">
        <!-- 变量快捷栏 -->
        <div class="var-bar">
        ...
        <div class="editor-toolbar" v-if="editor">
          ...
          <button type="button" @click="openTableDialog">▦ 表格</button>
          <button type="button" @click="toggleTableBorder" :disabled="!editor.isActive('table')">边框</button>
...
const handlePdfUpload = async (e) => {
  ...
  try {
    ...
    if (data.urls && data.urls.length) {
      // 一次性插入所有页面图片，用 insertContent 一次事务完成，避免逐张插入时光标停留覆盖的问题
...
.a4-content :deep(.ProseMirror) td.no-border,
.a4-content :deep(.ProseMirror) th.no-border {
